...
public class LambdasUsesWithJava8 {
    ...
    public static void main(String[] args) {
        ...
        /*LambdasUsesWithJava8 staticMyth = new LambdasUsesWithJava8();
    ...
    private void flatMapVsMap() {
       ...
       // words.stream().map(word -> word.split("")).flatMap(Arrays::stream).map(String::toUpperCase).distinct().forEach(System.out::println);
        ...
        List<int[]> pAndC = numbers.stream().sorted().flatMap(i -> numbers.stream().filter(j -> !j.equals(i) && j>i).map(k -> new int[]{i,k})).collect(Collectors.toList());
